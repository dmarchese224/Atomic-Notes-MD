...
function openMobileSidebar() { document.getElementById('sidebar').classList.add('mobile-open'); }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $$('.nav-item[data-view]').forEach(btn => {
    btn.addEventListener('click', () => { switchView(btn.dataset.view); autoCloseMobileSidebar(); });
  });

  // Open folder
  [$('openFolderBtn'), $('emptyOpenBtn'), $('settingsOpenBtn')].forEach(btn => {
    btn?.addEventListener('click', () => { openFolder(); autoCloseMobileSidebar(); });
  });
  $('refreshBtn').addEventListener('click', () => { loadAllFiles(); showToast('Refreshed'); autoCloseMobileSidebar(); });

  // New card
  $('newCardBtn').addEventListener('click', () => { openEditor(); autoCloseMobileSidebar(); });
  ...
  document.addEventListener('click', e => {
    ...
    const mobileBtn = $('mobileSidebarToggle');
    if (!isMobileView()) return;
    if (!sidebar.classList.contains('mobile-open')) return;
    const clickedInsideSidebar = sidebar.contains(e.target);
    const clickedToggle = mobileBtn.contains(e.target);
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape') {
      ...
      if ($('sidebar').classList.contains('mobile-open')) { closeMobileSidebar(); return; }
    }
    if (e.key === '?' && !inInput) {
      switchView('settings');
      return;
    ...
    window.addEventListener('load', async () => {
      ...
      for (const candidate of candidates) {
        try {
          ...
          console.warn('SW candidate failed:', candidate, err);
        }
      }
    });
  }

...
function closeEditor(){ $('editorOverlay').classList.remove('open'); closeWikilinkAutocomplete(); }
...
')) return null;
  return { start: open, cursor, query: afterOpen.trimStart() };
}

function getWikilinkMatches(query){
  const q = query.trim().toLowerCase();
  const cards = [...state.cards].sort((a,b)=>a.title.localeCompare(b.title));
  if (!q) return cards.slice(0,8);
  const starts = cards.filter(c=>c.title.toLowerCase().startsWith(q));
  const contains = cards.filter(c=>!c.title.toLowerCase().startsWith(q) && c.title.toLowerCase().includes(q));
  return [...starts, ...contains].slice(0,8);
}

function renderWikilinkAutocomplete(){
  const box = $('wikilinkAutocomplete');
  const ctx = getWikilinkContext();
  if (!ctx) { box.hidden = true; box.innerHTML = ''; wikilinkMatches = []; return; }
  wikilinkMatches = getWikilinkMatches(ctx.query);
  wikilinkActiveIndex = Math.min(wikilinkActiveIndex, Math.max(wikilinkMatches.length - 1, 0));
  if (!wikilinkMatches.length) {
    box.hidden = false;
    box.innerHTML = '<div class="wikilink-empty">No matching notes</div>';
    return;
  }
  box.hidden = false;
  box.innerHTML = wikilinkMatches.map((card, idx) => `
    <div class="wikilink-option ${idx===wikilinkActiveIndex?'active':''}" data-index="${idx}">
      <div>
        <div class="wikilink-option-title">${escHtml(card.title)}</div>
        <div class="wikilink-option-meta">${escHtml(card.type)}</div>
      </div>
      <div class="wikilink-option-meta">${card.tags.slice(0,2).map(escHtml).join(', ')}</div>
    </div>`).join('');
  box.querySelectorAll('.wikilink-option').forEach(el=>{
    el.addEventListener('mousedown', e=>{
      e.preventDefault();
      const idx = Number(el.dataset.index);
      insertWikilinkMatch(wikilinkMatches[idx]?.title);
    });
  });
}
...
function insertWikilinkMatch(title){
  if (!title) return;
  const ta = $('editorContent');
  const ctx = getWikilinkContext();
  if (!ctx) return;
  const before = ta.value.slice(0, ctx.start);
  const after = ta.value.slice(ctx.cursor);
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  $('newCardBtn').addEventListener('click', ()=>{ if(isMobile()) closeMobileSidebar(); openEditor(); });
  ...
  $('editorContent').addEventListener('input', ()=>{ wikilinkActiveIndex = 0; renderWikilinkAutocomplete(); });
  $('editorContent').addEventListener('click', renderWikilinkAutocomplete);
  $('editorContent').addEventListener('keyup', e=>{ if (!['ArrowUp','ArrowDown','Enter','Tab','Escape'].includes(e.key)) renderWikilinkAutocomplete(); });
  $('editorContent').addEventListener('keydown', e=>{
    const open = !$('wikilinkAutocomplete').hidden && wikilinkMatches.length;
    if (!open && e.key === 'Escape') { closeWikilinkAutocomplete(); return; }
    if (!open) return;
    if (e.key === 'ArrowDown') { e.preventDefault(); wikilinkActiveIndex = (wikilinkActiveIndex + 1) % wikilinkMatches.length; renderWikilinkAutocomplete(); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); wikilinkActiveIndex = (wikilinkActiveIndex - 1 + wikilinkMatches.length) % wikilinkMatches.length; renderWikilinkAutocomplete(); }
    else if (e.key === 'Enter' || e.key === 'Tab') { e.preventDefault(); insertWikilinkMatch(wikilinkMatches[wikilinkActiveIndex]?.title); }
    else if (e.key === 'Escape') { e.preventDefault(); closeWikilinkAutocomplete(); }
  });
